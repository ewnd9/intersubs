...
def mpv_pause():
	os.system('echo \'{ "command": ["set_property", "pause", true] }\' | socat - "' + config.mpv_socket + '" > /dev/null')

def mpv_resume():
	os.system('echo \'{ "command": ["set_property", "pause", false] }\' | socat - "' + config.mpv_socket + '" > /dev/null')

def mpv_pause_status():
	stdoutdata = subprocess.getoutput('echo \'{ "command": ["get_property", "pause"] }\' | socat - "' + config.mpv_socket + '"')
...
def mpv_fullscreen_status():
	stdoutdata = subprocess.getoutput('echo \'{ "command": ["get_property", "fullscreen"] }\' | socat - "' + config.mpv_socket + '"')
...
def mpv_message(message, timeout = 3000):
	os.system('echo \'{ "command": ["show-text", "' + message + '", "' + str(timeout) + '"] }\' | socat - "' + config.mpv_socket + '" > /dev/null')
	...
	def main(self):
		...
		while 1:
			...
			if inc * config.update_time > config.focus_checking_time - 0.0001:
				while 'mpv' not in subprocess.getoutput('xdotool getwindowfocus getwindowname') or (config.hide_when_not_fullscreen_B and not mpv_fullscreen_status()) or (os.path.exists(config.mpv_socket + '_hide')):
					if not was_hidden:
			...
			while tmp_file_subs != config.subs:
				if config.auto_pause == 2:
					if not auto_pause_2_ind and len(re.sub(' +', ' ', stripsd2(config.subs.replace('\n', ' '))).split(' ')) > config.auto_pause_min_words - 1 and not mpv_pause_status():
						mpv_pause()
						auto_pause_2_ind = 1
				...
				if config.auto_pause == 1:
					if len(re.sub(' +', ' ', stripsd2(config.subs.replace('\n', ' '))).split(' ')) > config.auto_pause_min_words - 1:
						mpv_pause()
...
class main_class(QWidget):
	...
	def render_subtitles(self, hide = False, redraw = False):
		...
		if redraw:
			...
			if config.split_long_lines_B and len(config.subs.split('\n')) == 1 and len(config.subs.split(' ')) > config.split_long_lines_words_min - 1:
